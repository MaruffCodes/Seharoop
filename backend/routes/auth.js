...
const generateDoctorId = async () => {
  ...
  return `DR${(count + 1).toString().padStart(4, '0')}`;
};
...
router.post('/register/patient', async (req, res) => {
  try {
    ...
    console.log(`✅ Patient registered: ${email} (ID: ${patientId})`);
    ...
    res.status(201).json({
      ...
      data: {
        token,
        user: { ...patient.toJSON(), hasMedicalForm: false },
        patientId,
      },
    });
  } catch (error) {
...
router.post('/register/doctor', async (req, res) => {
  try {
    ...
    if (!licenseNumber) {
      return res.status(400).json({ success: false, message: 'License / Registration Number is required' });
    }
    if (!medicalCouncilId) {
      return res.status(400).json({ success: false, message: 'Medical Council ID is required' });
    }
    if (password.length < 6) {
      return res.status(400).json({ success: false, message: 'Password must be at least 6 characters' });
    }
    ...
    // Check for duplicate license or council ID
    const dupLicense = await Doctor.findOne({ licenseNumber: licenseNumber.trim() });
    if (dupLicense) {
      return res.status(400).json({ success: false, message: 'A doctor with this License Number is already registered' });
    }
    const dupCouncil = await Doctor.findOne({ medicalCouncilId: medicalCouncilId.trim() });
    if (dupCouncil) {
      return res.status(400).json({ success: false, message: 'A doctor with this Medical Council ID is already registered' });
    }
    ...
    const doctor = new Doctor({
      ...
      medicalCouncilId: medicalCouncilId.trim(),
      hospitalName: hospitalName?.trim() || '',
      // Store hospital address as a structured or string field
      address: {
        clinic: hospitalName?.trim() || '',
        street: hospitalAddress?.trim() || '',
        country: 'India',
      },
      doctorId,
      role: 'doctor',
    ...
    console.log(`✅ Doctor registered: ${email} (ID: ${doctorId}, License: ${licenseNumber})`);
...
router.post('/login/patient', async (req, res) => {
  try {
    ...
    // Always check DB for hasMedicalForm so the client gets the correct value
    const medForm = await PatientMedicalForm.findOne({ patientId: patient._id });
    const hasMedicalForm = !!(medForm && medForm.completionStatus?.isComplete);

    // Sync hasMedicalForm on User model if out of date
    if (patient.hasMedicalForm !== hasMedicalForm) {
      await User.findByIdAndUpdate(patient._id, { hasMedicalForm });
    }

    const token = generateToken(patient._id, 'patient');

    console.log(`✅ Patient login: ${email} (hasMedicalForm: ${hasMedicalForm})`);
    ...
    res.json({
      ...
      data: {
        token,
        user: { ...patient.toJSON(), hasMedicalForm },
      },
    });
  } catch (error) {
    console.error('❌ Patient login error:', error);
...
router.post('/login/doctor', async (req, res) => {
  try {
    ...
    const doctor = await Doctor.findOne({ email: email.toLowerCase().trim(), role: 'doctor' });
    if (!doctor) {
      return res.status(401).json({ success: false, message: 'Invalid email or password' });
    }
    ...
    console.log(`✅ Doctor login: ${email} (ID: ${doctor.doctorId})`);

    res.json({
      success: true,
      message: 'Login successful',
      data: { token, user: doctor.toJSON() },
    });
  } catch (error) {
    console.error('❌ Doctor login error:', error);
    res.status(500).json({ success: false, message: 'Server error during login' });
