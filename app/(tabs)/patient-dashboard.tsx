...
//   // Updated openFile function using Sharing (most reliable)
//   const openFile = async (url: string, fileId?: string, fileType?: string, fileName?: string) => {
//     if (!url) {
//       Alert.alert('Not available', 'This file is still being processed.');
//       return;
...
//         if (!downloadedFile.exists) {
//           Alert.alert('Download Error', `Failed to download the file.\nCheck backend at ${BASE_URL}`);
//           return;
//         }
//       }

//       // Use Sharing for all file types - most reliable
//       const canShare = await Sharing.isAvailableAsync();
//       if (canShare) {
...
export default function PatientDashboard(): JSX.Element {
  ...
  const openFile = async (url: string, fileId?: string, fileType?: string, fileName?: string) => {
    ...
    try {
      ...
      if (!localFile.exists) {
        const token = await ApiService.getToken();
        await File.downloadFileAsync(fixedUrl, localFile, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
          idempotent: true,
        });
      }
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      {/* Views Modal - COMPLETELY FIXED ALIGNMENT */}
      <Modal animationType="slide" transparent visible={showViews} onRequestClose={() => setShowViews(false)}>
        <View style={s.overlay}>
          <View style={s.sheet}>
            <View style={s.viewsSheetHead}>
              <View style={s.viewsSheetHeadLeft}>
                <Text style={s.viewsSheetTitle}>Doctor Access History</Text>
                <Text style={s.viewsSheetSub}>{viewCount} total {viewCount === 1 ? 'view' : 'views'}</Text>
              </View>
              <TouchableOpacity onPress={() => setShowViews(false)} style={s.closeBtn}>
                <X size={20} color={C.textMid} strokeWidth={2} />
              </TouchableOpacity>
            ...
            <ScrollView
              style={s.viewsScrollView}
              contentContainerStyle={s.viewsSheetBody}
