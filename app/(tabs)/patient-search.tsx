...
//   // Record patient view when selected from search
//   const handlePatientSelect = async (patient: Patient) => {
//     try {
//       // Record this view for doctor analytics
//       await ApiService.recordPatientView(
//         patient._id,
//         patient.name,
//         patient.patientId
//       );
//       console.log('✅ Patient view recorded:', patient.name);
//     } catch (error) {
//       console.error('Error recording patient view:', error);
//     }
//     // Navigate to patient summary
//     router.push({
//       pathname: '/(tabs)/patient-summary',
//       params: { patientId: patient.patientId, patientName: patient.name }
//     });
//   };
...
//         setSearchQuery(patient.patientId);
//         Alert.alert('Patient Found', `Successfully scanned ${patient.name}.`);
...
//             patient.patientId
//           );
//           console.log('✅ Patient view recorded from QR scan:', patient.name);
//         } catch (error) {
//           console.error('Error recording patient view:', error);
//         }
//       } else {
...
//   const viewSummary = (patientId: string, patientName: string, patientObj: Patient) => {
//     // Record view before navigating
//     handlePatientSelect(patientObj);
//   };
...
//                 {/* CTA */}
//                 <TouchableOpacity style={s.viewBtn} onPress={() => handlePatientSelect(patient)} activeOpacity={0.85}>
//                   <Text style={s.viewBtnTxt}>View Medical Summary</Text>
//                   <ChevronRight size={15} color={C.primary} strokeWidth={2.5} />
//                 </TouchableOpacity>
//               </TouchableOpacity>
  ...
  const handleBarCodeScanned = async ({ data }: { type: string; data: string }) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // Navigate directly to summary after QR scan
        router.push({
          pathname: '/(tabs)/patient-summary',
          params: { patientId: patient.patientId, patientName: patient.name }
        });
