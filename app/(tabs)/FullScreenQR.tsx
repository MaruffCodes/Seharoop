...
// ✅ FIXED: Use legacy expo-file-system API (works in Expo Go SDK 53)
// The new { Directory, File, Paths } API from 'expo-file-system/next' is NOT
// available in Expo Go and crashes the entire tab layout silently.
...
type Specialty = 'general' | 'cardiology' | 'orthopedic';
interface SpecialtyQRData { qrCode: string; summary: any; specialty: Specialty; }

export default function FullScreenQR() {
  const { qrCodeUrl } = useLocalSearchParams();
  const [selectedSpecialty, setSelectedSpecialty] = useState<Specialty>('general');
  const [specialtyData, setSpecialtyData] = useState<Record<Specialty, SpecialtyQRData | null>>({
    general: null, cardiology: null, orthopedic: null,
  });
  const [loading, setLoading] = useState<Record<Specialty, boolean>>({
    general: true, cardiology: false, orthopedic: false,
  });
  const router = useRouter();
  const { userData } = useAuth();

  useEffect(() => { loadSpecialtyQR('general'); }, []);
  useEffect(() => {
    if (!specialtyData[selectedSpecialty] && !loading[selectedSpecialty]) {
      loadSpecialtyQR(selectedSpecialty);
    }
  ...
  const loadSpecialtyQR = async (specialty: Specialty) => {
    try {
      setLoading(prev => ({ ...prev, [specialty]: true }));
      if (specialty === 'general' && qrCodeUrl && !specialtyData.general) {
        setSpecialtyData(prev => ({ ...prev, general: { qrCode: qrCodeUrl as string, summary: null, specialty: 'general' } }));
        return;
      }
      const response = await ApiService.generateSpecialtyQR(specialty);
      if (response.success) {
        setSpecialtyData(prev => ({ ...prev, [specialty]: { qrCode: response.data.qrCode, summary: response.data.summary, specialty: response.data.specialty } }));
      }
    } catch (error) {
      console.error(`Error loading ${specialty} QR:`, error);
      Alert.alert('Error', `Failed to load ${specialty} QR code`);
  ...
  // ✅ FIXED: Use legacy FileSystem.downloadAsync + MediaLibrary.saveToLibraryAsync
  // instead of the new Directory/File/Paths API which crashes Expo Go
  const handleDownload = async () => {
    try {
      const { status } = await MediaLibrary.requestPermissionsAsync();
      if (status !== 'granted') {
      ...
      // Convert base64 data URI to file using legacy API
      const base64Data = currentData.qrCode.includes(',')
        ? currentData.qrCode.split(',')[1]
        : currentData.qrCode;
      ...
      Alert.alert('Saved ✓', `${selectedSpecialty} QR code saved to your photo library.`);
    } catch (error) {
      console.error('Error downloading QR code:', error);
      Alert.alert('Error', 'Failed to save QR code: ' + (error as Error).message);
    }
  ...
  const renderSpecialtyTab = (specialty: Specialty, label: string) => (
    <TouchableOpacity
      key={specialty}
      style={[styles.specialtyTab, selectedSpecialty === specialty && { backgroundColor: getSpecialtyColor(specialty), borderColor: getSpecialtyColor(specialty) }]}
      onPress={() => setSelectedSpecialty(specialty)}
    >
      <Text style={[styles.specialtyTabText, selectedSpecialty === specialty && styles.specialtyTabTextActive]}>
        {label}
      </Text>
      {loading[specialty] && (
        <ActivityIndicator size="small" color={selectedSpecialty === specialty ? '#FFF' : getSpecialtyColor(specialty)} style={{ marginLeft: 4 }} />
      )}
    </TouchableOpacity>
  );
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.qrContainer}>
          {currentData?.qrCode ? (
            ...
            <View style={styles.placeholderQR}>
              <ActivityIndicator size="small" color={currentColor} />
              <Text style={[styles.placeholderText, { color: currentColor }]}>Generating QR...</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Name:</Text>
            <Text style={styles.infoValue}>{basicInfo.patientDemographics?.name || basicInfo.patientInfo?.name || userData?.name || '—'}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Patient ID:</Text>
            <Text style={styles.infoValue}>{basicInfo.patientDemographics?.patientId || basicInfo.patientInfo?.patientId || userData?.patientId || '—'}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Blood Group:</Text>
            <Text style={[styles.infoValue, styles.bloodGroup]}>{basicInfo.medicalProfile?.bloodGroup || basicInfo.patientInfo?.bloodGroup || userData?.bloodGroup || 'Unknown'}</Text>
          </View>
          {basicInfo.documentCount !== undefined && (
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Documents:</Text>
              <Text style={styles.infoValue}>{basicInfo.documentCount}</Text>
            </View>
          )}
          {basicInfo.lastUpdated && (
            <View style={styles.infoRow}>
        ...
        <TouchableOpacity
          style={[styles.viewSummaryButton, { backgroundColor: currentColor }]}
          onPress={() => router.push({ pathname: '/(tabs)/patient-summary', params: { patientId: userData?.patientId } })}
        >
        ...
        <Text style={styles.note}>
          ...
          Tap above to view your complete medical history.
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  backButton: { padding: 8, borderRadius: 8, backgroundColor: '#EFF6FF' },
  title: { fontSize: 18, fontWeight: '700', color: '#1E293B' },
  downloadButton: { padding: 8, borderRadius: 8, backgroundColor: '#EFF6FF' },
  placeholder: { width: 40 },
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  loadingText: { marginTop: 12, fontSize: 16, color: '#64748B' },
  tabContainer: { flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 12, backgroundColor: '#FFF', gap: 8, borderBottomWidth: 1, borderBottomColor: '#E2E8F0' },
  specialtyTab: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 10, paddingHorizontal: 8, borderRadius: 8, backgroundColor: '#F1F5F9', borderWidth: 1, borderColor: '#E2E8F0' },
  specialtyTabText: { fontSize: 13, fontWeight: '600', color: '#64748B' },
  specialtyTabTextActive: { color: '#FFF' },
  scrollContent: { padding: 20 },
  qrContainer: { alignItems: 'center', marginBottom: 24, backgroundColor: '#FFF', borderRadius: 16, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
  qrImage: { width: 220, height: 220 },
  placeholderQR: { width: 220, height: 220, backgroundColor: '#F1F5F9', justifyContent: 'center', alignItems: 'center', borderRadius: 16, gap: 8 },
  placeholderText: { fontSize: 14, fontWeight: '500' },
  infoCard: { backgroundColor: '#FFF', borderRadius: 16, padding: 20, marginBottom: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 2 },
  infoCardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16, gap: 8, borderBottomWidth: 1, borderBottomColor: '#E2E8F0', paddingBottom: 12 },
  infoCardTitle: { fontSize: 16, fontWeight: '600', color: '#1E293B' },
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 },
  infoLabel: { fontSize: 14, color: '#64748B', fontWeight: '500' },
  infoValue: { fontSize: 14, color: '#1E293B', fontWeight: '600', flex: 1, textAlign: 'right' },
  bloodGroup: { color: '#2563EB', fontWeight: '700' },
  viewSummaryButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16, borderRadius: 12, gap: 8, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
  viewSummaryButtonText: { color: '#FFF', fontSize: 16, fontWeight: '600' },
  note: { fontSize: 12, color: '#9CA3AF', textAlign: 'center', fontStyle: 'italic', marginTop: 8, lineHeight: 18 },
});